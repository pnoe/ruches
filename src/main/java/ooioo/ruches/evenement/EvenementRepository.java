...
//	https://docs.spring.io/spring-data/rest/docs/current/reference/html/#customizing-sdr.configuring-the-rest-url-path
	...
	// Nombre d'événements faits dans l'année passée en paramètre et par type.
	@Query(value = """
			select count(*)
			  from Evenement
			  where type = :typEve
				and date_part('year', date) = :datea
			""")
	Integer countEveParAnneeEtType(int datea, TypeEvenement typEve);

	@Query(value = """
			select count(*)
			  from Evenement
			  where date_part('year', date) = :datea
			""")
	Integer countEveParAnnee(int datea);

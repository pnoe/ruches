...
public interface EssaimRepository extends CrudRepository<Essaim, Long> {
	...
	@Query(value = "select ruche.id, ruche.nom, rucher.id, rucher.nom from Essaim essaim left join Ruche ruche on ruche.essaim.id = essaim.id left join Rucher rucher on ruche.rucher.id = rucher.id "
			+ " where essaim.actif = ?1 order by essaim.nom")
	Iterable<Object[]> findRucheIdNomByActifOrderByNom(boolean actif);

	// liste ordonnée par nom d'essaims, des id et nom de la ruche associée
	@Query(value = "select ruche.id, ruche.nom, rucher.id, rucher.nom from Essaim essaim left join Ruche ruche on ruche.essaim.id = essaim.id left join Rucher rucher on ruche.rucher.id = rucher.id  order by essaim.nom")
	...
	@Query(value = "select essaim.id, essaim.nom from Essaim essaim left join Ruche ruche on ruche.essaim.id = essaim.id "
	+ " where ruche.essaim.id is null and essaim.actif is true order by essaim.dateAcquisition desc")
	Iterable<Object[]> findProjectedIdNomByRucheIsNullOrderByDateAcquisitionDesc();

	@Query(value = "select essaim from Essaim essaim where essaim.actif = 'true' and essaim not in (select essaim from Essaim essaim, Ruche ruche where ruche.essaim = essaim)")
	Iterable<Essaim> findEssaimByActifSansRuche();
	
	@Query(value = "select count(*) as nbessaims from Essaim where date_part('year', dateAcquisition)=?1")
